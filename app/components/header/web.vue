<template>
  <div>
    <v-navigation-drawer fixed v-model="drawerLeft" left temporary app>
      ...
      <v-divider></v-divider>
      <v-list dense>
        <v-list-tile @click.stop="left = !drawerLeft" v-for="(route) in routes" :key="route.key">
          <v-list-tile-content>
            <v-list-tile-title>
              <router-link
                :to="{path: route.path}"
              >{{ {state: translation, key: route.meta.translationKey} | translate}}</router-link>
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
...
import { getStore as getRouteStore } from "@common/router/store";
...
@Component({
  ...
  async created() {
    this.routes = await getRouteStore(this.$store).loadFilteredRoutes();
  }
})
